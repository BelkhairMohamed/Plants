...
class UserPlant {
    ...
    public function getStatsByRoom($userId) {
        $stmt = $this->db->prepare("
            SELECT room_location, COUNT(*) as count
            FROM user_plants
            WHERE user_id = ? AND room_location IS NOT NULL AND room_location != ''
    ...
    public function getPlantsByMonth($userId, $months = 6) {
        $stmt = $this->db->prepare("
